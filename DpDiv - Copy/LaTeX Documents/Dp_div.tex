...
\begin{document}
	...
	\indent From equation (1), $f$-divergences are a function of the distributions of the data from each class.\ In terms of the probability densities $p(x)$ and $g(x)$, the equation may be written as follows:\begin{equation}
		 D_\phi(p({x}),g({x})) = \int_{\Omega} f\bigg(\frac{p({x})}{g({x})}\bigg)g({x})dx
	...
	\\[0.5ex]
	\indent In [2] a modified version of this distance is proposed for implementation in binary classification tasks, as it leads to simple expressions for bounding the classification error rate in terms of the quantity. As a binary classification problems are considered in this work, a modified form of the distance, and its estimator are used:
	\begin{equation}
...
%	\\ [0.5ex]
	...
	\indent A common problem in machine learning is binary classification, in which data $\textbf{x}_i\in \mathbf{R^n}$ are assigned a class label $c_i \in \{0,1\}$.
