...
\begin{document}
	...
	%Choice of PHI
	\indent From equation (1), $f$-divergences are a function of the distributions of the data from each class.\ In terms of the probability densities, the equation may be rewritten as follows:\begin{equation}
		 D_\phi(p({x}),q({x})) = \int_{\Omega} f\bigg(\frac{p({x})}{q({x})}\bigg)q(\textbf{x})dx
	...
	\begin{tabular}[!h]{ |p{5cm}||p{4cm}|  }
		...
		\hline\hline
		K-L Divergence 	& $ -ln(x)$ \\
		$L^2$ Divergence & $ tlog(t)$ \\
		Total Variation Distance & $ \vert t-1 \vert$ \\
		Bhattacharya Distance & $(\sqrt{t}-1)^2$\\ 
		\hline 		
	\end{tabular}	
	\\ [0.5ex]
	\indent Note that for some cases the divergence may yield values that are not bounded depending on the choice of $\phi(x)$. An undefined or unbounded K-L divergence result can be problematic if we then apply the result to a task, and it may be desirable use a bounded divergence measure. 
	\\ [0.5ex]
	\indent Since direct evaluation of the expression containing these $\phi(x)$ is not possible, a number of estimation methods have been used to make the problem more tractable. Wang $et$ $al$. [27] derived a nonparametric divergence estimator based on first estimating the density ratio $\frac{dP}{dQ}$, and in [28] defined an  $k$-Nearest-Neighbors based divergence estimator that also requires estimates of a density ratio. But estimation of $\frac{dP}{dQ}$ instead of $dP$ and $dQ$ independently still poses the same drawback as estimating $dP$ and $dQ$: it is undesirable to estimate the divergence by performing the intermediate step of estimating the probability distribution. A key advantage of the $f$-divergence we consider is that can be estimated from the training data samples themselves, without independent steps. 
	\\ [0.5ex]	
	\indent Towards this end, P{\'o}czos $et al.$ [29] derive estimators for R{\'e}nyi and $L_2$ divergences based on $k$-Nearest Neighbors statistics, and apply the estimate to classifying astronomical data. Hero $et al.$ de
	...
	\indent Bootstrap resampling, first introduced by Efron in [10], is a powerful method to find the sampling distribution of an estimator. Given a data set of size $N$, bootstrap  straIn order to characterize the 
	\\ [0.5ex]
